...
export default function StudentCourseDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "content" | "assessments" | "resources"
  >("overview");

  const { useCourses, useStudentCourses, useAssessments, useAttendance } =
    useStudentQueries();
  ...
  const { data: studentCoursesResponse, isLoading: studentCoursesLoading } =
    useStudentCourses();
  const { data: assessmentsResponse, isLoading: assessmentsLoading } =
    useAssessments();
  const { data: attendanceResponse } = useAttendance();

  const isLoading =
    coursesLoading || studentCoursesLoading || assessmentsLoading;

  // Get the course data from API
  const course = useMemo(() => {
    if (!coursesResponse?.data || !studentCoursesResponse?.data?.data)
      return null;
    ...
    const moodleCourse = moodleCourses.find(
    ...
    const courseAssessments = (assessmentsResponse?.data || []).filter(
      (a: any) =>
        a.subject
          ?.toLowerCase()
          .includes(moodleCourse.fullname?.toLowerCase()) ||
        moodleCourse.fullname
          ?.toLowerCase()
          .includes(a.subject?.toLowerCase() || ""),
    );
      ...
      topics: [
        {
          ...
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 10
            : false,
          duration: "2 hours",
        },
        {
          ...
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 25
            : false,
          duration: "3 hours",
        },
        {
          ...
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 40
            : false,
          duration: "4 hours",
        },
        {
          ...
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 55
            : false,
          duration: "3 hours",
        },
        {
          ...
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 70
            : false,
          duration: "5 hours",
        },
        {
          ...
          title: "Final Project",
          completed: matchingStudentCourse?.progress
            ? matchingStudentCourse.progress > 85
            : false,
          duration: "10 hours",
        },
      ],
  ...
  if (!course) {
    return (
      <div className="space-y-6 font-outfit p-4 md:p-6">
  ...
  return (
    <div className="space-y-6 font-outfit p-4 md:p-6">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-primary p-6 text-white sm:p-8">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <Link href="/student/subjects">
                <Button variant="varsecondary" size="sm" className="gap-1">
                  <ArrowLeft size={16} />
                  Back
                </Button>
              </Link>
              <Badge className="border-white/25 bg-white/10 text-white">{course.code}</Badge>
              <Badge className={course.isEnrolled ? "border-transparent bg-accent text-white" : "border-white/25 bg-white/10 text-white"}>
                {course.isEnrolled ? "Enrolled" : "Available"}
              </Badge>
              {course.format && (
                <Badge className="border-white/25 bg-white/10 text-white">{course.format}</Badge>
              )}
            </div>
        ...
        <div className="relative mt-5 grid grid-cols-1 gap-3 sm:grid-cols-4">
          <div className="rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur-sm">
            <p className="text-[11px] text-white/70">Teacher</p>
            <p className="mt-1 text-sm font-semibold text-white">
              {course.teacher?.name || "Not Assigned"}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur-sm">
            <p className="text-[11px] text-white/70">Schedule</p>
            <p className="mt-1 text-sm font-semibold text-white">
              {course.schedule}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur-sm">
            <p className="text-[11px] text-white/70">Term</p>
            <p className="mt-1 text-sm font-semibold text-white">
              {course.term}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 p-3 backdrop-blur-sm">
            <p className="text-[11px] text-white/70">Progress</p>
            <p className="mt-1 text-sm font-semibold text-white">
              {course.progress}%
            </p>
          </div>
      ...
      >
        ...
        {/* Overview Tab */}
        <TabsContent value="overview" className="space-y-6 font-outfit">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {course.topics.map((topic, index) => (
                        <div
                          key={topic.id}
                          className="flex items-center justify-between p-3 rounded-xl border border-border/70 bg-background/60"
                        >
              ...
              {course.isEnrolled ? (
                <div className="space-y-4">
                  {course.materials.map((material: any) => (
                    <div
                      key={material.id}
                      className="flex items-center justify-between p-4 rounded-xl border border-border/70 bg-background/60"
                    >
                      <div className="flex items-center gap-3">
                        {material.type === "Video" ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground">
                    You need to enroll in this course to access learning
                    materials.
                  </p>
        ...
        <TabsContent value="assessments" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Track your performance in this course
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {assessment.type} • Due:{" "}
                          {assessment.due
                            ? formatDateString(assessment.due)
                            : "No due date"}
                        </p>
                      </div>

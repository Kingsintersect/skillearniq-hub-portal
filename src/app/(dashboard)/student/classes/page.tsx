...
function statusBadge(isEnrolled: boolean, progress?: number) {
    ...
    if (progress === 100) return <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200">Completed</Badge>;
    if (progress && progress > 0 && progress < 100) return <Badge className="bg-blue-100 text-blue-700 border-blue-200">In Progress</Badge>;
    return <Badge className="bg-amber-100 text-amber-700 border-amber-200">Not Started</Badge>;
}

function getGradeColor(grade?: string) {
    if (!grade || grade === 'N/A' || grade === 'In Progress') return 'text-muted-foreground';
    ...
    return (
        <div className="space-y-6 pb-6">
            ...
            <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                <h3 className="text-sm font-semibold text-foreground mb-2">Course Description</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                    <h3 className="text-sm font-semibold text-foreground mb-3">Course Information</h3>
                    <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">{course.visible ? 'Active' : 'Inactive'}</span>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="text-center p-2 bg-primary/5 rounded-lg">
                                    <p className="text-2xl font-bold text-foreground">{course.currentGrade}</p>
                                    <p className="text-xs text-muted-foreground">Current Grade</p>
                                </div>
                                <div className="text-center p-2 bg-primary/5 rounded-lg">
                                    <p className="text-2xl font-bold text-foreground">{course.assignments}</p>
                                    <p className="text-xs text-muted-foreground">Assignments</p>
                                </div>
                            </div>
                            {course.nextTopic && course.nextTopic !== 'Not Available' && course.nextTopic !== 'Start Learning' && (
                                <div className="p-3 bg-amber-500/10 rounded-lg">
                                    <p className="text-xs text-amber-600 font-semibold">Next Topic</p>
                ...
                {course.teacher && course.teacher.name !== 'Not Assigned' && (
                    <div className="rounded-2xl border border-border/70 bg-background/60 p-4">
                        <h3 className="text-sm font-semibold text-foreground mb-3">Teacher Information</h3>
            ...
            <div className="flex gap-3 pt-2">
                {course.isEnrolled ? (
                    <>
                        <Link href={course.lmsUrl || '#'} target="_blank" rel="noopener noreferrer" className="flex-1">
...
export default function StudentClassesPage() {
    ...
    const allCourses = useMemo(() => {
        return moodleCourses.map((moodleCourse: any) => {
            ...
            return {
                ...
                progress: progress,
                currentGrade: matchingStudentCourse?.currentGrade || (isEnrolled ? 'In Progress' : 'N/A'),
                ...
                nextTopic: matchingStudentCourse?.nextTopic || (isEnrolled ? 'Start Learning' : 'Not Available'),
    ...
    const filteredCourses = useMemo(() => {
        return allCourses.filter(course => {
            ...
            switch (statusFilter) {
                ...
                case 'enrolled': matchesStatus = course.isEnrolled === true; break;
                case 'available': matchesStatus = course.isEnrolled === false; break;
                case 'in-progress': matchesStatus = course.isEnrolled === true && course.progress !== undefined && course.progress > 0 && course.progress < 100; break;
                case 'completed': matchesStatus = course.isEnrolled === true && course.progress === 100; break;
                case 'not-started': matchesStatus = course.isEnrolled === true && (course.progress === 0 || course.progress === undefined); break;
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="flex items-center justify-center h-64">
                        <div className="text-center">
                            ...
                            <div className="text-lg text-muted-foreground">Loading your classes...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const completedCount = allCourses.filter(c => c.isEnrolled && c.progress === 100).length;
    const inProgressCount = allCourses.filter(c => c.isEnrolled && c.progress !== undefined && c.progress > 0 && c.progress < 100).length;
    const notStartedCount = allCourses.filter(c => c.isEnrolled && (c.progress === 0 || c.progress === undefined)).length;
        ...
        <div className="space-y-6 p-4 md:p-6">
            ...
            <section className="overflow-hidden rounded-3xl border border-primary/20 bg-linear-to-br from-primary/15 via-background to-cyan-500/10 p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        ...
                        <h1 className="mt-2 text-2xl font-bold text-foreground sm:text-3xl">My Classes</h1>
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">
                    <div className="rounded-2xl border border-border/70 bg-background/60 p-4 text-center">
                        ...
                        <p className="mt-1 text-2xl font-bold text-foreground">{enrolledCount}</p>
                    </div>
                    <div className="rounded-2xl border border-border/70 bg-background/60 p-4 text-center">
                    ...
                    <div className="rounded-2xl border border-border/70 bg-background/60 p-4 text-center">
                        ...
                        <p className="mt-1 text-2xl font-bold text-foreground">{notStartedCount}</p>
                    </div>
                    <div className="rounded-2xl border border-border/70 bg-background/60 p-4 text-center">
            ...
            {filteredCourses.length === 0 ? (
                ...
                <>
                                ...
                                <div className="rounded-3xl border border-border/70 bg-card p-5 shadow-sm hover:shadow-lg transition-all duration-300 hover:border-primary/30 h-full flex flex-col">
                                    ...
                                    <div className="flex items-start justify-between gap-2 mb-3">
                                        <div className="min-w-0 flex-1">
                                            <p className="text-xs font-mono text-muted-foreground truncate">{course.shortname}</p>
                                            <h3 className="text-base font-semibold text-foreground mt-0.5 line-clamp-1">{course.fullname}</h3>
                                        </div>
                                    ...
                                    {course.isEnrolled && (
                                        <div className="grid grid-cols-3 gap-2 mb-3">
                                            <div className="rounded-xl bg-muted/30 p-2 text-center">
                                                <p className="text-sm font-bold text-foreground">{course.assignments}</p>
                                                <p className="text-[10px] text-muted-foreground">Assignments</p>
                                            </div>
                                            <div className="rounded-xl bg-muted/30 p-2 text-center">
                                                <p className={`text-sm font-bold ${getGradeColor(course.currentGrade)}`}>
                                                    {course.currentGrade === 'In Progress' ? '--' : course.currentGrade}
                                                </p>
                                                <p className="text-[10px] text-muted-foreground">Grade</p>
                                            </div>
                                            <div className="rounded-xl bg-muted/30 p-2 text-center">
                                                <p className="text-sm font-bold text-foreground">{course.materialsCount}</p>
                                    ...
                                    <div className="mt-4 pt-3 border-t border-border/50 grid grid-cols-2 gap-2">
                                        {course.isEnrolled ? (
                                            <>
                                                <Link href={course.lmsUrl || '#'} target="_blank" rel="noopener noreferrer" className="col-span-1">
                                                    <Button 
                                                        variant="default" 
                    ...
                    <div id="grid-sentinel" className="h-4 flex items-center justify-center">
                        {isFetchingNextPage && (
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead className="border-b border-border/70 bg-muted/30">
                                    <tr>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Course</th>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Code</th>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Status</th>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Progress</th>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Teacher</th>
                                        <th className="text-left p-4 text-sm font-semibold text-foreground">Actions</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {paginatedCourses.map((course) => (
                                        <tr key={course.id} className="border-b border-border/70 hover:bg-muted/30 transition-colors">
                                            <td className="p-4">
                                            ...
                                            <td className="p-4">
                                                <div className="text-sm">{course.teacher?.name !== 'Not Assigned' ? course.teacher?.name : 'N/A'}</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-2">
                                                    {course.isEnrolled && (
                                                        <Link href={course.lmsUrl || '#'} target="_blank" rel="noopener noreferrer">
            ...
            <Drawer open={!!selectedCourse} onOpenChange={(open) => !open && setSelectedCourse(null)}>
                <DrawerContent className="w-full max-w-full rounded-none border-0 h-[95vh]">
                                ...
                                <>
                                    <DrawerHeader className="px-0 border-b border-border/70 pb-4">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex items-center gap-3">
                                                {statusBadge(selectedCourse.isEnrolled || false, selectedCourse.progress)}

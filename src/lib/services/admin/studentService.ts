// src/lib/services/studentService.ts - UPDATE COMPLETE FILE

import { ApiResponse } from '@/types/auth';
import { apiClient } from '@/core/client';

// Add these interfaces at the top of the file (before the service methods):
...
// Keep all your existing interfaces (StudentProfile, GamificationProfile, etc.)
// ... all your existing interfaces remain here ...

/**
 * Domain types - Your existing interfaces
 */
...
export const studentService = {
  ...
  getCourseGrades: async (courseId: number): Promise<ApiResponse<CourseGradesResponse>> => {
    try {
      const response = await apiClient.get<any>(`/student/course/course-gradings/${courseId}`);
